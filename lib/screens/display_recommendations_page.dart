  ...
  Future<void> _generateNewRecommendations() async {
    try {
      final response = await http.post(
        Uri.parse(
            'http://localhost:5000/generate-recommendations/${widget.userId}'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Failed to generate new recommendations')),
          );
        }
      }
    } catch (e) {
          ...
          IconButton(
            ...
            onPressed: () async {
              try {
                final response = await http.post(
                  Uri.parse(
                      'http://localhost:5000/refresh-recommendations/${widget.userId}'),
                );
                if (response.statusCode == 200) {
                  final newRecommendations = json.decode(response.body);
                  if (context.mounted) {
